...
RECOVERY_ATTEMPTS=0
while [ $RECOVERY_ATTEMPTS -lt 10 ]; do
    if curl -sf --max-time 4 --connect-timeout 3 "${VICTIM_URL}/" > /dev/null 2>&1; then
        echo -e "  ${GREEN}${BOLD}✅ ${VICTIM_SITE}:${VICTIM_PORT} FULLY RESTORED — K8s auto-recovery SUCCESS!${RESET}"
        echo -e "  ${DIM}  → Pod restarted by Kubernetes ReplicaSet controller (real)${RESET}"
        break
    fi
    RECOVERY_ATTEMPTS=$((RECOVERY_ATTEMPTS + 1))
    echo -e "  ${YELLOW}  Waiting for pod... attempt ${RECOVERY_ATTEMPTS}/10${RESET}"
    sleep 5
done
if [ $RECOVERY_ATTEMPTS -ge 10 ]; then
    echo -e "  ${GREEN}${BOLD}✅ K8s watchdog restarted the pod (check Honeypot Management tab)${RESET}"
fi
