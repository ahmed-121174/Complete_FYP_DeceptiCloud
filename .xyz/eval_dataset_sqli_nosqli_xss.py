#!/usr/bin/env python3
"""
Evaluation with TensorFlow 2.20.0 on Datasets/SQLI-nosqli-XSS
"""
...
print("EVALUATION ON Datasets/SQLI-nosqli-XSS")

# Load dataset

print("\n Loading dataset from Datasets/SQLI-nosqli-XSS...")
dataset_path = Path("Datasets/SQLI-nosqli-XSS")
csv_files = list(dataset_path.rglob("*.csv"))

dataframes = []
for csv_file in csv_files:
    # Skip files that might be duplicates or processed versions if needed,

    # but for now load everything recursively as requested.
...
label_col = next((col for col in combined_df.columns if 'label' in col.lower()), None)

if not text_col or not label_col:
    print(f" Could not identify text or label columns. Found: {combined_df.columns}")
    sys.exit(1)
...
if label_set <= {'0', '1'}:
    ...
    # heuristic: if 'benign' is present, use it. otherwise assume first unique is benign?

    # Let's try to match the logic used before which seemed standard for this dataset

    benign_values = [v for v in unique_labels if 'benign' in str(v).lower()]
    if benign_values:
        benign_label = benign_values[0]
    else:
        benign_label = unique_labels[0] # Fallback
    
    print(f"   Assuming Benign Label: {benign_label}")
...
try:
    ...
    print(f" Error loading/running model: {type(e).__name__}: {str(e)}")
    import traceback
    traceback.print_exc()
