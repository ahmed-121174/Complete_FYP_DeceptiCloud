...
banner() {
    ...
    echo -e "    ML Detection + Honeypot Deception System  (FYP Jury Demo)         "
    echo -e "  ${RESET}"
...
ml_classify() {
    ...
    routed=$(echo "$result" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('routed_to','?'))" 2>/dev/null || echo "?")
    ...
    if [ "$is_mal" = "True" ]; then
        ...
        echo -e "  ${DIM} ${name}: ${verdict} (${conf})${RESET}"
    fi
}
...
PROXY_BASE=$(curl -sf http://localhost:8080/proxy/status 2>/dev/null | \
...
PROXY_AFTER=$(curl -sf http://localhost:8080/proxy/status 2>/dev/null | \
    python3 -c "import sys,json;d=json.load(sys.stdin);a=d.get('attacker_summary',{});print(sum(v.get('count',0) for v in a.values()))" 2>/dev/null || echo "?")
NEW_ATTACKS=$((PROXY_AFTER - PROXY_BASE))

# FINAL SUMMARY

echo -e "\n${CYAN}${BOLD}"

echo -e "                DECEPTION COMPLETE — SUMMARY                   "
...
LLM_STATS=$(curl -sf --max-time 5 http://localhost:8080/proxy/status 2>/dev/null | \
    python3 -c "
import sys,json
...
echo -e "    ${BOLD} Dashboard:${RESET}${CYAN}   http://localhost:9000   (admin / DeceptiCloud)  "
echo -e "  ${RESET}\n"
